...
Например: несколько полей, объединенных в сущность (DTO) для отправки в сервис расчета графика платежей.
...
Также **DTO** могут использоваться для слоя отображения (View).  
...
Как правило нет необходимости в отображении полной сущности со всеми данными на стороне клиента. Поэтому данные возвращаются 
в "урезанном" виде, который ровно совпадает с тем, что пользователь должен увидеть в текущий момент на своей стороне на экране.
...
сущность, содержащую поля со списками продавцов и заявителей.

### Service
Какие же сущности отвечают за выполнение всей бизнес логики в нашем приложении?  
Такие классы именуются **Service** (Сервис).
...
Он будет содержать CRUD методы, в рамках которых может выполнять дополнительная логика, например проверка корректности при создании нового ипотечного займа.
...
Для этого мы должны обратиться из `MortgageService` в другой сервис - `ExternalEvaluationService` который в свою очередь 
сформирует специальную **DTO**, которую отправит в совершенно другое приложение, которое произведет расчет и вернет нам результат 
обратно через `ExternalEvaluationService` в `MortgageService` и которое мы запишем в нашу сущность `Mortgage`.

### Repository (DAO)
Также с уровня **Service** мы можем обращаться на уровень **Repository** (Репозиторий).  
Это необходимо для того чтобы сохранить в базу или получить из базы какие-либо данные.
